...
class NPNet(nn.Module):
      def __init__(self, model_id, pretrained_path=True, device='cuda',enable_cross=True,enable_finetune=False,enable_adapter=True,enable_dropout=True) -> None:
            ...
            if self.enable_cross:
                  self.cross_attn = CrossAttention(dim=2048 if model_id != 'DiT' else 1024)
      ...
      def get_model(self):
            ...
            unet_embedding = NoiseTransformer(resolution=128,enable_finetune=self.enable_finetune,enable_adapter=self.enable_adapter,enable_dropout=self.enable_dropout).to(self.device).to(torch.float32)
            unet_svd = SVDNoiseUnet(resolution=128,enable_drop=self.enable_dropout).to(self.device).to(torch.float32)
      ...
      def forward(self, initial_noise, prompt_embeds):
            ...
            if self.enable_cross:
                  golden_embedding = self.cross_attn(golden_embedding, text_emb)
