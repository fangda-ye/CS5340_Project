...
try:
    from model.rnn_seq_model_v3 import NoiseSequenceRNN_v3 # Import the V3 RNN model
...
def main(args):
    ...
    # Save tokenizer for later checking text length
    tokenizer, tokenizer_2 = None, None
    max_token_length = 77  # Default SDXL max token length
    ...
    try:
        if "xl" in args.base_model_id.lower() and "hunyuan" not in args.base_model_id.lower():
            ...
            # Save tokenizer for checking text length
            tokenizer = base_pipeline.tokenizer
            tokenizer_2 = base_pipeline.tokenizer_2
            max_token_length = tokenizer.model_max_length  # Usually 77
    ...
    # --- Add function to check text length ---
    def check_token_length(prompt):
        if tokenizer is None:
            return True, 0  # If no tokenizer, allow all text by default
        
        # Check the first tokenizer
        token_ids = tokenizer(prompt, return_tensors="pt").input_ids
        token_length = token_ids.shape[1]
        
        # If there is a second tokenizer (SDXL specific), check it too
        if tokenizer_2 is not None:
            token_ids_2 = tokenizer_2(prompt, return_tensors="pt").input_ids
            token_length_2 = token_ids_2.shape[1]
            token_length = max(token_length, token_length_2)
            
        return token_length <= max_token_length, token_length
    ...
    saved_checkpoints = []
    # Track statistics for overly long prompts
    total_prompts = 0
    skipped_prompts = 0
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(model):
                    ...
                    # --- Check the length of each prompt ---
                    valid_indices = []
                    for i, prompt in enumerate(prompts):
                        total_prompts += 1
                        is_valid, token_length = check_token_length(prompt)
                        if is_valid:
                            valid_indices.append(i)
                        else:
                            skipped_prompts += 1
                            if accelerator.is_main_process:
                                accelerator.print(f"Skipping prompt with token length {token_length} > {max_token_length}: '{prompt[:50]}...'")
                    
                    # If no valid prompts, skip the entire batch
                    if len(valid_indices) == 0:
                        accelerator.print(f"Warning: Entire batch at step {step} contains only prompts that exceed max token length. Skipping.")
                        continue
                    
                    # Keep only valid samples
                    if len(valid_indices) < len(prompts):
                        filtered_prompts = [prompts[i] for i in valid_indices]
                        filtered_source_noise = source_noise[valid_indices]
                        filtered_golden_sequence = golden_sequence[valid_indices]
                    else:
                        filtered_prompts = prompts
                        filtered_source_noise = source_noise
                        filtered_golden_sequence = golden_sequence
                    
                    B, T_target, C, H, W = filtered_golden_sequence.shape
                    input_noise_seq = torch.cat([filtered_source_noise.unsqueeze(1), filtered_golden_sequence[:, :-1]], dim=1)
                    target_noise_seq = filtered_golden_sequence # Target is the full sequence x'_1 to x'_n

                try:
                    with torch.no_grad(): text_embed = encode_text(filtered_prompts).to(device)
                except Exception as e: 
                    accelerator.print(f"Error encoding prompts: {e}")
                    continue
                ...
                avg_nll_loss = accelerator.gather(torch.tensor(nll_loss_val, device=device).repeat(B)).mean()
                train_nll_loss += avg_nll_loss.item() / args.gradient_accumulation_steps
                if args.kl_weight > 0:
                        avg_kl_loss = accelerator.gather(torch.tensor(kl_loss_val, device=device).repeat(B)).mean()
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % args.log_steps == 0:
                        ...
                        # Calculate skip percentage safely
                        skip_percentage = (skipped_prompts / total_prompts * 100) if total_prompts > 0 else 0.0
                        logs = {
                            "step_loss": train_loss / num_opt_steps_so_far, 
                            "nll_loss": train_nll_loss / num_opt_steps_so_far, 
                            "epoch": epoch + 1,
                            "prompts_skipped": f"{skipped_prompts}/{total_prompts} ({skip_percentage:.2f}%)"
                        }
        ...
        accelerator.print(f"Prompts skipped due to length: {skipped_prompts}/{total_prompts} ({skip_percentage:.2f}%)")
