...
# Import RNN model and sequence dataset loader
try:
    from model import NoiseSequenceRNN # Import the RNN sequence model
...
# --- Loss Function (Copied from Transformer training script) ---
def gaussian_nll_loss(mean_pred, log_var_pred, target, reduction='mean'):
    ...
    inv_var = torch.exp(-log_var_pred)
...
def main(args):
    ...
    if accelerator.is_main_process: accelerator.init_trackers("rnn_seq_model_training")
    ...
    # (Same logic as train_seq_model.py)
    ...
    try:
        ...
        if "xl" in args.base_model_id.lower() and "hunyuan" not in args.base_model_id.lower():
            ...
            def encode_text(prompt_list):
                    with torch.no_grad():
                        # Assume we use pooled output for RNN conditioning [B, D_txt]
                        ...
                        return pooled_prompt_embeds # Return pooled
            del base_pipeline.vae, base_pipeline.unet
                ...
                def encode_text(prompt_list):
                    with torch.no_grad():
                        ...
                        # Assume pooled/single vector output if available, otherwise first element
                        return embed_output # Adjust based on actual output
    ...
    # --- Initialize Noise Sequence RNN Model ---
    accelerator.print("Initializing NoiseSequenceRNN model...")
    # Determine text_embed_dim based on loaded encoder(s)
    if args.npnet_model_id == 'DiT': text_embed_dim = 1024 # Placeholder
    else: text_embed_dim = 1280 # Placeholder for SDXL pooled output (CLIP-G) - ADJUST IF NEEDED!
    ...
    # Determine max sequence length for dataloader based on dataset 'num_steps'
    try:
            meta_df_peek = pd.read_csv(metadata_path, nrows=1)
            dataset_num_steps = meta_df_peek['num_steps'].iloc[0]
            dataloader_max_seq = dataset_num_steps # Load full sequences from dataset
    ...
    for epoch in range(args.num_epochs):
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % args.save_steps == 0:
                        if accelerator.is_main_process:
                            save_path = os.path.join(args.output_dir, f"rnn_model_step_{global_step}.pth")
        ...
        if accelerator.is_main_process:
            epoch_save_path = os.path.join(args.output_dir, f"rnn_model_epoch_{epoch+1}.pth")
    ...
    if accelerator.is_main_process:
        final_save_path = os.path.join(args.output_dir, "rnn_model_final.pth")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Noise Sequence RNN Model")
    # Dataset Args
    parser.add_argument("--dataset_dir", type=str, required=True, help="Path to NPD sequence dataset.")
    # max_seq_len is now determined from dataset metadata
    ...
    parser.add_argument("--text_embed_dim", type=int, default=0, help="Dimension of text embeddings (0 to infer from base_model_id).")
    ...
    parser.add_argument("--cnn_base_filters", type=int, default=32, help="Base filters for CNN encoder/decoder.")
    parser.add_argument("--cnn_num_layers", type=int, default=4, help="Layers in CNN encoder/decoder.")
    ...
    # Training Args
    parser.add_argument("--output_dir", type=str, default="rnn_seq_model_output", help="Output directory.")
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size per GPU.") # Reduced default BS
    parser.add_argument("--gradient_accumulation_steps", type=int, default=2, help="Gradient accumulation.") # Increased default grad acc
